...
# The "Whitelist"
# These are senders we TRUST, so we exclude them from the report to reduce noise.
$ExemptList = @(
    "ceo@domain1.com",
...
###### Core Logic #####
#if subject line shows "undisclosed", make sure you modify Exchange Transport Rule settings and restart service. (get-transportservce |Select-Object name, messagetrackinglogsubjectloggingenabled)
$TOPTRAFFIC = Get-ExchangeServer | Get-messagetrackinglog -Source SMTP -resultsize unlimited -start $startdate -end $enddate |  

    #Filte #1: We only look for RECEIVE events because it counts the number of times a device connected to the server
    #this is better for filtering out events of misconfigured alerts
    Where-Object 
    {$_.eventid -eq "RECEIVE" -and 
     $_.sender -like "*@domain1.com" -and 
     $_.sender -ne $ExpemptList
     }  |
     
    #All of the above messages are stores in $TOPTRAFFIC and are then stripped down to WHEN(time stamp) WHO(sender,receiver), WHY (subject).
